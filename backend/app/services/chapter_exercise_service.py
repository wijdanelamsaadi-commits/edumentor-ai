...
EXERCISE_BANK_VERSION = "regional-bank-v2-balanced-20260801"
...
def normalize_bank_exercise(item: dict, learner_level: str, position: int) -> dict:
    ...
    choices = balance_choice_position(
        choices,
        correct_answer,
...
def balance_choice_position(
    ...
    *,
    ...
    """Place la bonne réponse à des positions A/B/C/D équilibrées et stables."""
    ...
    # Position stable pour une même série, mais répartie entre A/B/C/D.
    desired_index = (max(position, 1) - 1 + int(chapter_id or 0)) % len(choices)
    reordered = list(distractors)
    reordered.insert(desired_index, correct_choice)
    return reordered
