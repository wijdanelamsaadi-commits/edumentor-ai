...
def get_student_preparation(db: Session, student: UserProfile) -> dict:
    ...
    return {
        ...
        "weak_points": weak_points(db, regional_attempts),
...
def submit_regional_exam(db: Session, student: UserProfile, exam_id: int, payload: dict) -> dict:
    ...
    for question in active_questions(assessment):
        ...
        evaluation = score_regional_answer(question, selected)
...
def serialize_regional_attempt(db: Session, attempt: AssessmentAttempt, include_correction: bool) -> dict:
    ...
    for question in active_questions(assessment):
        ...
        if include_correction and infer_question_type(question) in {"response_long", "production_ecrite"}:
            row["teacher_validation_message"] = "Réponse enregistrée — validation enseignante nécessaire"
...
def score_regional_answer(question: AssessmentQuestion, selected: str) -> dict:
    ...
    points = float(question.points or 1)
    if not selected:
        return {"correct": False, "points_awarded": 0.0}
    if question_type == "qcm":
        correct = normalize_text(selected) == normalize_text(question.correct_answer)
        return {"correct": correct, "points_awarded": points if correct else 0.0}
    expected = expected_elements(question)
    if not expected:
        correct = normalize_text(selected) == normalize_text(question.correct_answer)
        return {"correct": correct, "points_awarded": points if correct else 0.0}
    selected_norm = normalize_text(selected)
    matched = sum(1 for item in expected if normalize_text(item) and normalize_text(item) in selected_norm)
    ratio = matched / len(expected)
    return {
        "correct": ratio >= 0.6,
        "points_awarded": round(points * ratio, 2),
    }
