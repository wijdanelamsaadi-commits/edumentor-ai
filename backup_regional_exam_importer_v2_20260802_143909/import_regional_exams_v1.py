...
def load_models() -> dict[str, type]:
    ...
    for name in required:
        ...
        models[name] = model
    for optional in ("Course", "Skill", "AssessmentAttempt", "AssessmentAnswer"):
...
def find_course_id(db: Session, models: dict[str, type], work_title: str) -> Any:
    ...
    candidates = exact or [row for row in works if needle in normalize(scalar_attr(row, "title", "name")) or normalize(scalar_attr(row, "title", "name")) in needle]
    ...
    if Course is not None:
        ...
        exact_courses = [row for row in courses if normalize(scalar_attr(row, "title", "name")) == needle]
        fuzzy_courses = exact_courses or [row for row in courses if needle in normalize(scalar_attr(row, "title", "name"))]
        if fuzzy_courses:
            return getattr(fuzzy_courses[0], "id")
    available = ", ".join(str(scalar_attr(row, "title", "name")) for row in works[:20])
    raise RuntimeError(f"Œuvre/cours introuvable pour '{work_title}'. Œuvres disponibles: {available}")
...
def assessment_values(payload: dict[str, Any], course_id: Any) -> dict[str, Any]:
...
def import_dataset(
    ...
    for payload in payloads:
        ...
        course_id = find_course_id(db, models, work_title)
        ...
        values = assessment_values(payload, course_id)
...
def main() -> int:
    ...
    try:
        ...
        if args.dry_run:
            ...
            db.commit()
            print("IMPORT RÉUSSI")
        print(json.dumps({"dataset_version": DATASET_VERSION, **stats, "verification": verification}, ensure_ascii=False, indent=2))
        return 0
